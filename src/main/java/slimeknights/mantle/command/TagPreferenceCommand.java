...
public class TagPreferenceCommand {
  ...
  private static <T> int runGeneric(CommandContext<CommandSourceStack> context, Registry<T> registry) {
    ...
    if (preference == null) {
      context.getSource().sendSuccess(() -> Component.translatable(EMPTY_TAG, registry.key().location().toString(), name.toString()), true);
      return 0;
    } else {
      context.getSource().sendSuccess(() -> Component.translatable(PREFERENCE, registry.key().location().toString(), name.toString(), registry.getKey(preference).toString()), true);
      return 1;
    }
  }
}
